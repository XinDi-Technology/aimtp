...
const preRenderMermaid = async (html: string): Promise<string> => {
  ...
  for (const match of matches) {
    ...
    try {
      let svg = await renderMermaidSync(code);

      // 修复：Mermaid 11 生成的 SVG 根元素有 data-id 但没有 id，
      // 导致内部 CSS 选择器 #mermaid-xxx 无法命中，消息线等样式失效。
      // 这里将 data-id 同步为 id，使内部样式表生效。
      const dataIdMatch = svg.match(/<svg[^>]*?\sdata-id="([^"]+)"/);
      if (dataIdMatch && !/<svg[^>]*?\sid=/.test(svg)) {
