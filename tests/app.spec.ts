...
test.describe('Aimtp Application', () => {
  ...
  test.describe('Core UI Components', () => {
    ...
    test('should have all core components visible', async ({ page }) => {
      ...
      await expect(page.locator('.editor-preview')).toBeVisible({ timeout: UI_TIMEOUT });
  ...
  test.describe('Settings Panel', () => {
    ...
    test('should have paragraph spacing control', async ({ page }) => {
      await expect(page.locator('[data-testid="settings-panel"]')).toContainText(/段落间距/, { timeout: UI_TIMEOUT });
    });
  ...
  test.describe('Preview Rendering', () => {
    test('should show pages in preview', async ({ page }) => {
      const previewFrame = page.frameLocator('.editor-preview iframe').first();
      await previewFrame.locator('.pagedjs_page').first().waitFor({
        state: 'attached',
        timeout: UI_TIMEOUT,
      });
      
      const pages = previewFrame.locator('.pagedjs_page');
  ...
  test.describe('Page Stacking Mode', () => {
    test('should show pagedjs pages container', async ({ page }) => {
      const previewFrame = page.frameLocator('.editor-preview iframe').first();
      await previewFrame.locator('.pagedjs_pages').waitFor({ state: 'visible', timeout: UI_TIMEOUT });
      const pagesContainer = previewFrame.locator('.pagedjs_pages');
